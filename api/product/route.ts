...
export const getProductList = (req: ProductListRequest, res: Response): Response | void => {
    ...
    db.getAllProduct(status, seller_id, async (rest: any, err: Error) => {
        ...
        const productList = await Promise.all(rest.map(async (product) => {
            const sellerName = await new Promise((resolve) => {
...
export const productReview = (req: ProductReviewRequest, res: Response): Response | void => {
    ...
    db.getProduct(product_id, status ?? null, st, (rest: any, err: Error) => {
        ...
        db.reviewProduct(product_id, rest.product_name, rest.description, rest.price, rest.category, rest.images, rest.seller.seller_id, rest.like, rest.view, rest.interaction, rest.release_date, action, message ?? null, (rest: any, err: Error) => {
            if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
...
export const productUpload = (req: ProductUploadRequest, res: Response) => {
    const { server_id, seller_id, name, description, category, price } = req.body;
    const files = (req as any).files;
    if (!helper.detectParam(server_id, seller_id)) {
        return helper.response(
            res, 400, false,
            errors[400]['400.parameter'].message.replace(`{PARAMETER}`, `server_id, seller_id`),
    ...
    try {
        db.getUserData(seller_id, async (result: any, err: Error) => {
            ...
            db.addProduct(null, name!, description!, price!, category!, results, seller_id, (rest: any, err: Error) => {
                if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
                if (!rest) return helper.response(res, 404, false, errors[404]['404.product'].message, errors[404]['404.product'].code);
...
export const productUpdate = async (req: ProductUpdateRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        if (!['approved', 'rejected', 'pending'].includes(status)) {
            return helper.response(res, 400, false, 'Parameter status hanya bisa "approved" atau "rejected".', errors[400]['400.error'].code);
        }
                ...
                await Promise.all(
                    keys.map(async (f, i) => {
                        ...
                        if (['like', 'interaction'].includes(f)) {
                            dt = await new Promise<number>((resolve, reject) => {
                                db.getProduct(product_id, status, false, (rest: any, err: Error) => {
                                    if (err) return reject(err);
                                    resolve(Number(rest[f]) + 1);
                                });
                            });
                        ...
                        await new Promise<void>((resolve, reject) => {
                            db.updateProduct(product_id, status, dt, f, (rest: any, err: Error) => {
                                if (err) return reject(err);
                                if (!rest) return reject(new Error(errors[404]['404.product'].message));
                                if (rest == 'not_found') return reject(new Error(`Field ${f} tidak ditemukan di database.`));
                                resolve();
                            });
                        });
                    })
                ...
                oldFileIDs.map(async (file_id) => {
                    ...
                    await new Promise<void>((resolve, reject) => {
                        db.updateProductImage(product_id, status, 'remove', file_id, null, null, (rest: any, err: Error) => {
                            if (err) return reject(err);
                            if (!rest) return reject(new Error(`File ${file_id} tidak ditemukan di database.`));
                            resolve();
                        });
                    });
                })
                ...
                await Promise.all(
                    uploadResults.map(result =>
                        new Promise<void>((resolve, reject) => {
                            db.updateProductImage(product_id, status, 'add', result.file_id, null, result.file_name, (rest: any, err: Error) => {
                                if (err) return reject(err);
                                resolve();
                            });
                        })
...
export const productRemove = (req: ProductRemoveRequest, res: Response): Response | void => {
    ...
    db.getUserData(seller_id, (result: any, err: Error) => {
        ...
        db.getProduct(product_id, status, false, (rest: any, err: Error) => {
            if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
            if (!rest) return helper.response(res, 404, false, errors[404]['404.user'].message, errors[404]['404.user'].code);
