...
const updateAccount = (req, res) => {
    ...
    if (!/(set|remove)/i.exec(action)) return helper.response(res, 400, false, 'Aksi tidak valid, tersedia: set, remove', errors[400]['400.error'].code);
    ...
    if (field.includes('profile_photo') && action != 'remove') return helper.response(res, 400, false, 'Anda hanya bisa menggunakan endpoint ini dengan aksi remove jika field adalah profile_photo', errors[400]['400.error'].code);
    ...
    db.getUserData(ident, (result, err) => {
        ...
        if (field == 'password') {
            helper.pwd('dec', String(password), result.password, (isValid, err) => {
                if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
                if (!isValid) return helper.response(res, 401, false, errors[401]['401.password'].message, errors[401]['401.password'].code);

                helper.pwd('enc', String(new_value), null, (rests, err) => {
                    if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
                    db.updateUserData(ident, action, rests, field, (rest, err) => {
                        if (err) return helper.response(res, 400, false, err, errors[400]['400.error'].code);
